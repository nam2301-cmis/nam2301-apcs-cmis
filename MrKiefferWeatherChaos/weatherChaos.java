    ...
    public static void main ( String[] args)
        ...
        if (user > 0 && user <= 31)
            ...
            for (int i = 0; i < temps.length; i++)
                ...
                day = i + 1; // starting from 1
                if (i > 0)
                    {
                    swing = Math.abs(temps[i] - temps[i - 1]); //getting the difference between two cosecutive temperature
                    }//end if
                else
                    {
                    swing = 0; //fot the first day
                    }//end else
                if (temps[i] < 0)
                    {
                ...
                if (temps[i] > max)
                    {
                    max = temps[i]; // getting the temp
                    maxDay = i + 1; //getting the day
                    }//end if
                if (temps[i] < min)
                    {
                    min = temps[i];
                ...
                total += temps[i]; //empty string is above 
